#!/usr/bin/env python3
"""
Run comprehensive netball analysis with court calibration AND OCR jersey reading.
"""
...
def run_calibrated_ocr_analysis(video_path: str, config_path: str, output_dir: str, 
                               start_time: float = 0.0, end_time: float = None,
                               save_video: bool = False, enable_ocr: bool = True):
    """Run comprehensive analysis with calibration AND OCR."""
    
    logger.info("=== STARTING CALIBRATED OCR NETBALL ANALYSIS ===")
    ...
    ocr_config = OCRProcessingConfig(
        ...
        team_assignment_threshold=0.7
    )
    
    ocr_analyzer = OCRNetballAnalyzer(config_path, ocr_config, enable_ocr=enable_ocr)
    ...
    if save_video:
        ...
        video_output_path = output_path / f"calibrated_ocr_analysis_{int(start_time)}_{end_time_str}.mp4"
    ...
    # Setup CSV files
    detections_csv = output_path / "calibrated_ocr_detections.csv"
    ...
    det_writer.writerow([
        ...
        'jersey_number', 'jersey_confidence', 'team_assignment'
    ])
    
    stats_writer.writerow([
        'frame_number', 'timestamp', 'players_count', 'balls_count', 'hoops_count',
        'calibrated_players', 'calibrated_balls', 'calibrated_hoops',
        'zone_violations', 'calibration_status', 'jersey_detections', 'confident_players'
    ])
    ...
    while cap.isOpened() and frame_count < (end_frame - start_frame):
        ...
        ocr_result = ocr_analyzer.process_frame(frame, current_frame_number, current_timestamp)
        ...
        jersey_detections_count = len(ocr_result["ocr_results"])
        confident_players_count = len([j for j in ocr_result["jersey_data"] if j["is_confident"]])
        ...
        total_confident_players += confident_players_count
        ...
        stats_writer.writerow([
            ...
            violations_count, result.calibration_status, jersey_detections_count, confident_players_count
        ])
        
        # Process detections with OCR data
        for player in result.players:
            ...
            # Simple matching based on bbox overlap (could be improved with proper tracking)
            for ocr_data in ocr_result["jersey_data"]:
                # This is a simplified matching - in production you'd use proper tracking
                if ocr_data["player_id"] == object_id_counter % len(ocr_result["jersey_data"]):
                    jersey_number = ocr_data["jersey_number"]
                    jersey_confidence = ocr_data["confidence"]
                    team_assignment = ocr_data["team_assignment"]
                    break
            ...
            det_writer.writerow([
                ...
                jersey_number or "", jersey_confidence or "", team_assignment or ""
            ])
        ...
        if save_video:
            ...
            for i, player in enumerate(result.players):
                ...
                if i < len(ocr_result["jersey_data"]) and ocr_result["jersey_data"][i]["jersey_number"]:
                    jersey_text = f"#{ocr_result['jersey_data'][i]['jersey_number']}"
                    cv2.putText(vis_frame, jersey_text, (x1, y1 - 35), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
            ...
            ocr_text = f"Jersey Detections: {jersey_detections_count} | Confident: {confident_players_count}"
            ...
            cv2.putText(vis_frame, violations_text, (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    ...
    # Write jersey data
    jersey_analysis = ocr_analyzer.get_jersey_analysis()
    for player_data in jersey_analysis.get("confident_players", []):
        jersey_writer.writerow([
            player_data["player_id"],
            player_data["jersey_number"],
            player_data["confidence"],
            player_data["team_assignment"],
            "", "", "", player_data.get("is_confident", False)
        ])
    ...
    # Generate summary
    logger.info(f"\n=== CALIBRATED OCR ANALYSIS SUMMARY ===")
    ...
    logger.info(f"\nOCR totals:")
    logger.info(f"  Jersey detections: {total_jersey_detections}")
    logger.info(f"  Confident players: {total_confident_players}")
    logger.info(f"  Team assignments: {len(jersey_analysis.get('team_assignments', {}))}")
    ...
    logger.info(f"  Jersey data: {jersey_data_csv}")
    logger.info(f"  Zone stats: {zone_stats_csv}")
    if save_video:
        logger.info(f"  Video: {video_output_path}")
    
    logger.info("=== CALIBRATED OCR ANALYSIS COMPLETED ===")


def main():
    parser = argparse.ArgumentParser(description="Run calibrated netball analysis with OCR")
    parser.add_argument('--video', type=str, required=True, help='Path to input video')
    parser.add_argument('--config', type=str, default='configs/config_enhanced.json', help='Configuration file')
    parser.add_argument('--output', type=str, default='output/calibrated_ocr_analysis', help='Output directory')
